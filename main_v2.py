...
async def get_info_about_vc(vc_name, url, browser) -> tuple:
    try:
        ...
        print(f'Execution time of scrapping vc website: {end_scrape_text-start_scrape_text}')
...
async def get_info_about_vc_portfolio_startups(portfolio_url, max_portfolios=10, browser=None) -> list:
    try:
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for vc_data in data_from_sheets:
            ...
            processed_data = await process_vc_data(vc_name, vc_website_url, vc_portfolio_url, vc_linkedin_url, analyst_name, analyst_email, vc_stages, vc_industries, browser)
