...
def get_info_about_vc(vc_name, url, browser) -> tuple:
    try:
        with Manager() as manager:
            shared_text = manager.list()
            pool = Pool()
            pool.apply_async(scrape_website, args=[url, browser, shared_text])
            pool.close()
            pool.join()
            print(shared_text)
            raw_text = shared_text[0] if shared_text else ""
...
def get_info_about_vc_portfolio_startups(portfolio_url, max_portfolios=10, browser=None) -> list:
    try:
        ...
        partial_func = partial(scrap_portfolio_website, browser)
        portfolio_startups_links = get_portfolio_links(portfolio_url, browser)

        p = Pool()
        result = p.map(partial_func, portfolio_startups_links)
        p.close()
        p.join()
