...
from constants.response_schemas import LAST_RESPONSE_INDIVIDUAL_EMAIL

# Default output format
output_parser = CommaSeparatedListOutputParser()
format_instructions = output_parser.get_format_instructions()

# Individual email output format
...
STAGES_INPUT = STAGES_PROMPT.format(subject="stages")

LAST_ANSWER = PromptTemplate(
...
CHECK_IF_FUND_WILL_INVEST_IN_USER_STARTUP_PROMPT = PromptTemplate(
    template="""You work for a venture capital fund that is considering investing in startups. The fund has a portfolio of various startups in different industries, and it evaluates potential investments based on the problems the startups solve. The key information you have about the startups is the vc_portfolio_startup_solution string, which describes the core problem each startup addresses.

Your task is to determine whether the venture capital fund will invest in a user's startup solely based on whether it solves problem from very similar industry as at least one startup in the fund's investment portfolio. If the user's startup meets this criterion, the fund will invest in it.

You have access to two sets of information:

Information about the user's startup:
{question}

Information about the venture capital fund's investment portfolio:
{context}

Based on the rules given to you above, if you think that fund will invest in user startup YOU MUST RETURN EXACTLY "Yes. Startup Name 1, Startup Name 2, ...", where "Startup Name 1, Startup Name 2, ..." is the name(s) of startups from fund's invest portfolio thar are you consider as `similar to user's startup`
In any other way, please return just "No" and nothing else
""",
    input_variables=["question", "context"],
...
#
#
# LAST_ANSWER_WITH_STARTUPS_NAMES_PROMPT_v2 = PromptTemplate(
#     template="""CONTEXT:
# {context}
#
...
# ------
# If the venture capital fund has invested in a startup(s) that solves similar problems to the startup described below, then the fund will invest in the startup.
# ------
#
# ------
# Startup Description:
...
# ------
#
# If you think that the fund has in its investment portfolio startup(s) that solve a similar problem to the startup described below, you MUST answer like said in OUTPUT FORMAT below.
# ------
# OUTPUT FORMAT:
# ------
# {format_instructions}
...
# How to determine whether a fund will invest in a startup?
# ------
# If a venture fund has already invested in a startup(s) that solves a problem(s) similar to the problem(s) solved by the startup(s) described below, the fund will invest in startup described below.
# ------
#
# If you think that the fund will invest in startup, then YOU MUST ANSWER "YES".
# In any other way, please return just "No"
#
# ------
# Startup Description:
